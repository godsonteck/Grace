...
import {
  ...
  Printer, CreditCard, ShoppingCart, ArrowRight, RefreshCw, Wifi, WifiOff, BarChart3, TrendingUp, Monitor, HardDrive, ShieldCheck, ClipboardList, Briefcase, UserPlus, FileText, Activity, LogOut, Microscope, MapPin, ChevronDown
} from "lucide-react";
...
export default function AdminPage() {
  ...
  const [modalType, setModalType] = useState<"record" | "staff" | "equipment" | "report">("record");
  ...
  }, [records, filteredAppointments, filteredInvoices, filteredStaff, filteredEquipment]);
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans antialiased text-secondary">
      ...
      <main className="flex-grow min-w-0 overflow-y-auto h-screen custom-scrollbar">
        ...
        <div className="p-10 pb-20">
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {[
                  ...
                  <div key={i} className="bg-white p-8 rounded-[40px] border border-white shadow-[0_20px_50px_rgba(0,0,0,0.04)] flex flex-col justify-between hover:scale-[1.05] transition-all cursor-default">
                    <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center mb-8 shadow-inner", stat.bg, stat.color)}>
               ...
               <table className="w-full text-left">
                  <thead>
                    <tr className="bg-slate-50 text-[10px] font-black uppercase text-slate-400 tracking-[0.3em]">
                      ...
                      <th className="px-12 py-8">Verification Status</th>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {filteredAppointments.map(apt => (
                      <tr key={apt.id}>
                        ...
                        <td className="px-12 py-8">
                           {apt.reportAttached ? (
                             <span className="flex items-center gap-3 text-green-600 text-[10px] font-black uppercase italic"><ShieldCheck className="h-5 w-5" /> Result Attested</span>
                           ) : (
            ...
            <div className="bg-white rounded-[50px] shadow-2xl border border-white overflow-hidden animate-in fade-in duration-700">
               <div className="p-12 border-b bg-slate-50/30 flex justify-between items-center">
                  ...
                  <button onClick={() => handleOpenStaffModal()} className="bg-secondary text-white px-10 py-5 rounded-[28px] font-black uppercase tracking-[0.2em] text-[10px] flex items-center gap-3 hover:bg-primary transition-all shadow-2xl">
                    <UserPlus className="h-5 w-5" /> Execute Recruitment
                  </button>
               </div>
               <table className="w-full text-left">
                  <tbody className="divide-y divide-slate-50">
                    {filteredStaff.map(s => (
                      <tr key={s.id}>
                        ...
                        <td className="px-12 py-8 text-right"><button onClick={() => handleOpenStaffModal(s)} className="p-4 bg-slate-100 rounded-2xl hover:bg-primary hover:text-white transition-all shadow-sm"><Edit2 className="h-4 w-4" /></button></td>
                      </tr>
                    ))}
                  </tbody>
               </table>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {records.map(r => (
                      <tr key={r.id}>
                        ...
                        <td className="px-12 py-8 text-center font-black text-secondary text-xl">${r.price}</td>
                        <td className="px-12 py-8 text-right">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {filteredEquipment.map(e => (
                      <tr key={e.id}>
                        ...
                        <td className="px-12 py-8 text-right"><button onClick={() => handleOpenEquipModal(e)} className="p-4 bg-slate-100 rounded-2xl hover:bg-primary hover:text-white transition-all shadow-sm"><Edit2 className="h-4 w-4" /></button></td>
                      </tr>
                    ))}
                  </tbody>
               </table>
          ...
          {activeTab === "logs" && (
             <div className="bg-white rounded-[50px] shadow-2xl border border-white p-12">
                ...
                <div className="space-y-4">
                   {auditLogs.map(log => (
                     <div key={log.id} className="grid grid-cols-4 gap-4 p-8 rounded-[35px] bg-slate-50 border-2 border-slate-100 text-[10px] font-black uppercase tracking-widest italic">
                        <div className="text-slate-300">{log.timestamp}</div>
                        <div className="text-primary">{log.module}</div>
                        <div className="text-secondary">{log.action}</div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-secondary/95 backdrop-blur-3xl z-[200] flex items-center justify-center p-6">
          <div className="bg-white rounded-[70px] shadow-2xl w-full max-w-3xl overflow-hidden animate-in fade-in zoom-in duration-500">
            ...
            <form onSubmit={handleSave} className="p-20 space-y-16">
               {modalType === "report" && (
                 <div className="space-y-12">
                    <div className="p-10 rounded-[45px] bg-blue-50 border-4 border-blue-100 italic">
                       <p className="text-xs font-black text-blue-400 uppercase tracking-[0.5em] mb-6 italic">Clinical Intelligence</p>
                       <p className="text-4xl font-black text-blue-900 tracking-tighter uppercase leading-none">{editingItem.patientName}</p>
                       <p className="text-sm font-bold text-blue-700 mt-4 italic">{editingItem.scanName} • Pipeline Node: #{editingItem.id}</p>
                    </div>
                    <div className="space-y-6">
                       <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.5em] ml-6 italic">Findings & Impression</label>
                       <textarea required rows={5} className="w-full px-12 py-10 rounded-[50px] border-4 border-slate-50 focus:border-primary outline-none font-black text-2xl italic bg-slate-50/50 shadow-inner resize-none transition-all" value={reportContent} onChange={e => setReportContent(e.target.value)} />
                    </div>
                 </div>
               )}
               ...
               {modalType === "record" && (
                 <div className="space-y-12">
                   ...
                   <div className="grid grid-cols-2 gap-12">
                      <div className="space-y-6">
                        <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.5em] ml-6 italic">Fee ($)</label>
                        <input type="number" className="w-full px-12 py-10 rounded-[50px] border-4 border-slate-50 focus:border-primary outline-none font-black text-3xl italic text-primary" value={recordForm.price} onChange={e => setRecordRecordForm({...recordForm, price: parseInt(e.target.value) || 0})} />
                      </div>
                   </div>
                 </div>
               )}
               ...
               <button type="submit" className="w-full bg-secondary text-white py-12 rounded-[55px] font-black uppercase tracking-[0.3em] text-2xl flex items-center justify-center gap-8 hover:bg-primary transition-all shadow-[0_50px_100px_rgba(0,0,0,0.25)] active:scale-95 italic">
                  <ShieldCheck className="h-12 w-12 text-primary" /> Attest & Save
               </button>
            </form>
          </div>
        </div>
